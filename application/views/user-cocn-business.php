<!DOCTYPE html>
<html>
<head>
<?php include_once('templete/pub_head_london.php') ?>
</head>
<body>
<?php include_once('templete/pub_nav_inside.php') ?>
<!-- 用户中心开始 -->
<div class="container-full">
  <?php include_once('templete/pub_leftnav.php') ?>
  <div class="view-frt">
    <div class="manage-panel">
      <div class="sub-nav-panel">
        <span class="title sub f16">杭州名商网络有限公司官网
          <a href="/page/user-cocn" class="fl-r f14 mr0">
            <i class="ico-blue-arrow-back"></i>
            返回公司官网
          </a>
        </span>
        <p class="mt5">
          <a href="/page/user-cocn-settings" class="">通用设置</a>
          <a href="/page/user-cocn-news" class="">新闻中心</a>
          <a href="/page/user-cocn-product" class="">产品中心<i class="badge">
              <font>99</font>
            </i>
          </a>
          <a href="/page/user-cocn-comment" class="">用户评论
            <i class="badge">
                <font>99</font>
              </i>
          </a>
          <a href="/page/user-cocn-business" class="on">工商信息</a>
          <a href="/page/user-cocn-limit" class="">权限设置</a>
        </p>
      </div>
      <div class="mt25 after-cls">
        <div class="install-box">
          <div class="auto-table">
            <table width="100%" class="">
              <tbody>
                <tr class="">
                  <td width="140" align="center" rowspan="1" class="bg-5897cd">
                    <span class="title col-white lh24 mt10 mb10 in-block pos-rela">
                      公司名称
                    </span>
                  </td>
                  <td width=" " class="pl25 bg-fa">
                    <div class="col-default f16 lh30">
                      <a href="javascript:;" class="col-default ta-r mr5 in-block pl5">杭州名商网络有限公司</a>
                      <em class="block-a-zaiye">在业</em>
                    </div>
                  </td>
                  <td width="20%" class="pl25 bg-fa">
                    <a href="javascript:;" class="col-default f12 in-block ml20">更新
                    </a>
                  </td>
                </tr>
              </tbody>
            </table>
          </div>
        </div>
        <div class="install-box mt10">
          <div class="auto-table">
            <table width="100%" class="">
              <tbody>
                <tr class="">
                  <td width="140" align="center" rowspan="4" class="bg-5897cd">
                    <span class="title col-white mt10 mb10 in-block pos-rela">
                    基础信息
                    </span>
                  </td>
                  <td width="" class="pl10 bg-fa">
                    <dl class="info">
                      <dt>类型：</dt>
                      <dd class="lh22">有限责任公司（自然人投资或控股）
                      </dd>
                    </dl>
                  </td>
                  <td width="" class="pl10 bg-fa">
                    <dl class="info">
                      <dt>住所：</dt>
                      <dd class="lh22">浙江省杭州市拱墅区上塘路329号11层
                      </dd>
                    </dl>
                  </td>
                  <td width="20%" class="pl25 bg-fa">
                    <a href="/page/user-cocn--set" class="col-default f12 in-block ml20">设置<i class="ico-blue-arrow ml3"></i>
                    </a>
                  </td>
                </tr>
                <tr>
                  <td width="" class="pl10 bg-fa">
                    <dl class="info">
                      <dt>法定代表：</dt>
                      <dd>林赞松</dd>
                    </dl>
                  </td>
                  <td class="pl10 bg-fa">
                    <dl class="info">
                      <dt>注册资本：</dt>
                      <dd>壹佰万元整</dd>
                    </dl>
                  </td>
                  <td class="pl10 bg-fa">

                  </td>
                </tr>
                <tr>
                  <td class="pl10 bg-fa">
                    <dl class="info">
                      <dt>成立日期：</dt>
                      <dd>2019年1月2日</dd>
                    </dl>
                  </td>
                  <td class="pl10 bg-fa">
                    <dl class="info">
                      <dt>营业期限：</dt>
                      <dd>2019年1月2日 至 长期</dd>
                    </dl>
                  </td>
                  <td class="pl10 bg-fa">
                  </td>
                </tr>
                <tr>
                  <td colspan="3" class="pl10 bg-fa">
                    <dl class="info">
                      <dt>经营范围：</dt>
                      <dd class="width-auto">
                          计算机软件、通讯设备、自动化控制设备
                      </dd>
                    </dl>
                  </td>
                </tr>
              </tbody>
            </table>
          </div>
        </div>
        <div class="install-box mt10">
          <div class="auto-table">
            <table width="100%" class="">
              <tbody>
                <tr class="">
                  <td width="140" align="center" rowspan="1" class="bg-5897cd">
                    <span class="title col-white lh24 mt10 mb10 in-block pos-rela">
                      股东及出资信息
                    </span>
                  </td>
                  <td width=" " class="pl25 bg-fa">
                    <em class="block-a-zanwu">暂无</em>
                  </td>
                  <td width="20%" class="pl25 bg-fa">
                    <a href="/page/user-cocn--set" class="col-default f12 in-block ml20">设置<i class="ico-blue-arrow ml3"></i>
                    </a>
                  </td>
                </tr>
              </tbody>
            </table>
          </div>
        </div>
        <div class="install-box mt10">
          <div class="auto-table">
            <table width="100%" class="">
              <tbody>
                <tr class="">
                  <td width="140" align="center" rowspan="1" class="bg-5897cd">
                    <span class="title col-white lh24 mt10 mb10 in-block pos-rela">
                      对外投资和分析
                    </span>
                  </td>
                  <td width=" " class="pl25 bg-fa">
                    <em class="block-a-zanwu">暂无</em>
                  </td>
                  <td width="20%" class="pl25 bg-fa">
                    <a href="/page/user-cocn--set" class="col-default f12 in-block ml20">设置<i class="ico-blue-arrow ml3"></i>
                    </a>
                  </td>
                </tr>
              </tbody>
            </table>
          </div>
        </div>
      </div>

    </div>
  </div>
</div>
<!-- footer -->
<?php include_once('templete/pub_foot_inside.php') ?>
<script src="/htdocs/js/slick/slick.min.js?<?php echo CACHE_TIME; ?>"></script>
<link rel="stylesheet" href="/htdocs/js/slick/slick.css?<?php echo CACHE_TIME; ?>">
<script type="text/javascript">
$(function(){

})
</script>
</body>
</html>
